...
async def test_prompt_names_the_current_channel(bot):
    ...
    service = make_service(client)
    await service.ask(context_for(bot), "hello", channel_id=424242)
    system = client.calls[0][0]["content"]
    assert "<#424242>" in system and "RIGHT NOW" in system
    assert "NEVER tell someone to take a conversation to the channel" in system
